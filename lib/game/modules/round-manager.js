...
.defines(function() {
    ...
    RoundManager = ig.Class.extend({
        ...
        // gameover bg
        gameover: new ig.Image('media/gameoverBig.png'),
       ...
       bgMusic: new ig.Sound( 'media/sounds/themeMusic.mp3', false ),
        ...
        /**
         * The mode we're playing
         */
        mode: null,

        /**
         * The level if playing in B mode
         */
        level: null,
        ...
        update: function() {
            ...
            switch(this.roundState) {
                case RoundManager.state.GAME_OVER:
                    ...
                    break;
                case RoundManager.state.VICTORY:
                    this.handleGameOverControls();
                    break;
        ...
        draw: function() {
            ...
            // draw scoreboard while playing but hide when gameover
            this.futura.draw(this.stage,834 , 80, ig.Font.ALIGN.LEFT);
            this.futura.draw(this.lineCount, 834, 240, ig.Font.ALIGN.LEFT);
            this.futura.draw(this.score, 834, 400, ig.Font.ALIGN.LEFT);    
            }
        ...
        handleGameOver: function() {
           ...
           // this.font.draw('GAME OVER', 96, 64, ig.Font.ALIGN.CENTER);
            this.gameover.draw(-270, 0);
            // ig.system.context.fillStyle = "rgb(155,155,155)";
            //  ig.system.context.rect(800,50,50,500);
        ...
        handleCleanLines: function() {
            if (this.timerDestruction.delta() >= 0) {
                ...
                if (this.mode == 'B' && this.lineCount == 0) {
                    this.handleModeBLineEndGameCondition();
                } else {
        ...
        handleGameOverControls: function() {
            if (ig.input.pressed('rotateRight') ||
                ...
                ig.input.pressed('enter') || ig.input.pressed('go')) {
                ig.game.endRound();
            }
        },
        ...
        lockActiveBlock: function( forcefulSound ) {
            ...
            console.log('full lines completed = ' + countFullLines);
            ...
            if (countFullLines > 0) {
                ...
                if (this.mode == 'A') {
                    ...
                    this.lineCount = Math.max(0, this.lineCount - countFullLines);
                }
        ...
        generateBlock: function() {
            ...
            switch (num) {
                case 1:
                    ...
                    entity = ig.game.spawnEntity( EntityBlockShapeLine, ig.global.SPAWN_POS_X + 50, ig.global.SPAWN_POS_Y + yOffset +28 );
                    
                    break;
                case 2:
                    xOffset = ig.global.TILESIZE; // positive offset
                    entity = ig.game.spawnEntity( EntityBlockShapeSquare, ig.global.SPAWN_POS_X + xOffset -16 , ig.global.SPAWN_POS_Y );
                ...
                case 3:
                    ...
                    yOffset = ig.global.TILESIZE * -1;
                    entity = ig.game.spawnEntity( EntityBlockShapeL, ig.global.SPAWN_POS_X + 50, ig.global.SPAWN_POS_Y + yOffset + 8 );
                    break;
                case 4:
                    // LR requires offset, because it has an empty row in its shape
                    yOffset = ig.global.TILESIZE * -1;
                    entity = ig.game.spawnEntity( EntityBlockShapeLR, ig.global.SPAWN_POS_X + 16, ig.global.SPAWN_POS_Y + yOffset -8  );
                    break;
                case 5:
                    entity = ig.game.spawnEntity( EntityBlockShapeS, ig.global.SPAWN_POS_X - 16, ig.global.SPAWN_POS_Y );
                    break;
                case 6:
                // line requires offset, because it has two empty rows in its shape
                    yOffset = ig.global.TILESIZE * -2;
                    ...
                    break;
                case 7:
                      entity = ig.game.spawnEntity( EntityBlockShapeSR, ig.global.SPAWN_POS_X + 8, ig.global.SPAWN_POS_Y );
                      break;  
                default:
                    entity = ig.game.spawnEntity( EntityBlockShapeSR, ig.global.SPAWN_POS_X + 8, ig.global.SPAWN_POS_Y );
                    break;
        ...
        /**
         * Generate noise blocks at the start of a game in B mode.
         */
        generateNoiseBlocks: function() {
            ig.log("GENERATE NOISE BLOCKS");
            for (var y = ig.global.FIELD_ROWS - this.level * ig.global.NOISE_ROWS_PER_LEVEL; y < ig.global.FIELD_ROWS; y++) {
                var blocksInRow = 0;
                for (var x = 1; x <= ig.global.FIELD_COLS; x++) {
                    if (Math.random() < ig.global.NOISE_CHANCE && blocksInRow < ig.global.FIELD_COLS - 1) {
                        entity = ig.game.spawnEntity( EntityBlockShapeSingle, ig.global.TILESIZE * x, ig.global.TILESIZE * y, { color: (Math.floor(Math.random() * 7) + 1).toString() } );
                        this.fieldManager.lockBlock( entity );
                        blocksInRow++;
                    }
                }
            }
        ...
        handleModeBLineEndGameCondition: function() {
            this.roundState = RoundManager.state.VICTORY;
            this.soundGameOver.play();
            if (this.playBgMusic) {
                ig.music.fadeOut(1);
            }
        ...
        startRound: function( mode, startStage, startLevel ) {
            this.mode = mode;
            if (this.mode == 'B') {
                this.lineCount = ig.global.MODEB_INIT_ROWS;
